...
async function blobUpload(filename, base64Data) {
    try {
        ...
        if (!r.ok)
            return { ok: false, error: data.error || `Blob error (${r.status})` };
...
async function blobFetch(url) {
    try {
        ...
        if (!r.ok || !data.content)
            return { ok: false, error: data.error || 'No content' };
...
async function loadPendingUploads() {
    ...
    if (label)
        label.textContent = items.length ? `(${items.length})` : '';
    if (!items.length) {
...
async function commitFileToGitHub(filePath, base64Content) {
    ...
    if (!getRes.ok) {
        showStatus(`✗ Could not read destination: ${getRes.error}`);
        return false;
    }
    ...
    if (!putRes.ok) {
        showStatus(`✗ Could not publish file: ${putRes.error}`);
        return false;
    }
    return true;
}
...
function uploadFileDrop(e) {
    ...
    const files = Array.from(e.dataTransfer.files).filter(f => {
        if (f.size > MAX_FILE_SIZE) {
            showStatus(`✗ "${f.name}" exceeds 25 MB limit.`);
            return false;
        }
        return true;
    });
...
async function _finalizeUploadImpl() {
    ...
    if (isAdmin()) {
        ...
        let msg = '';
        if (successes.length)
            msg += `✓ ${successes.length} file${successes.length > 1 ? 's' : ''} published to "${dest || 'repository root'}".`;
        if (failures.length)
        ...
        let msg = '';
        if (successes.length)
            msg += `✓ ${successes.length} file${successes.length > 1 ? 's' : ''} submitted for review.${dest ? ` Destination: ${dest}.` : ''}`;
        if (failures.length)
            msg += `${msg ? '\n' : ''}✗ ${failures.length} failed: ${failures.map(r => r.name).join(', ')}`;
        showUploadResultUI(failures.length === 0, msg.trim());
        updatePendingBadge();
...
function showUploadResultUI(ok, msg) {
    ['us1', 'us2', 'us3'].forEach(id => { const el = document.getElementById(id); if (el)
