        ...
        if (target) {
            ...
            (async function loadAndRenderTree() {
                try {
                    ...
                    // Prefer existing aside#subject-tree .tree-body when present
                    const aside = target.querySelector('#subject-tree') || document.querySelector('#subject-tree');
                    const container = aside ? (aside.querySelector('.tree-body') || aside) : (target.querySelector('#subjectTree') || (function () {
                    ...
                    function renderNodeAsList(node, parentList, repoRootName) {
                        if (!node)
                            return;
                        const li = mk('li', 'sidebar-tree-item ' + (node.type === 'folder' ? 'folder' : 'file'));
                        const row = mk('div', 'sidebar-tree-row');
                        if (node.type === 'folder') {
                            "use strict";
                            ...
                            function parseSubjectRepos(raw) {
                            const map = {};
                            String(raw || '')
                            .split(',')
                            ...
                            async function getSubjectRepoMap() {
                            if (!subjectRepoMapPromise) {
                            subjectRepoMapPromise = fetch('/api/config')
                            .then((res) => (res.ok ? res.json() : Promise.resolve({})))
                            .then((data) => parseSubjectRepos(data?.SUBJECT_REPOS || ''))
                            .catch(() => ({}));
                            }
                            ...
                            */
                            function renderSubjectTree(container, nodes) {
                            ...
                            function wireSubjectSampleLinks(container, repo, branch) {
                            ...
                            async function populateSubjectTree(container, slug) {
                            ...
                            window.initSubjectShell = initSubjectShell;
