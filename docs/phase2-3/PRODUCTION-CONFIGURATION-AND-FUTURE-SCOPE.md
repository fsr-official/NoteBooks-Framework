...
The Vercel runtime evidence showed `ERR_REQUIRE_ESM` while loading `@octokit/rest` from the CommonJS-emitted `src/api/_shared.js`. The deployment was using ESM-only `@octokit/rest` 22.x and `@octokit/auth-app` 8.x packages.[7] [8] Vercel’s CommonJS function wrapper cannot `require()` those releases. The fix pins `@octokit/rest` to 20.0.2 and `@octokit/auth-app` to 6.0.0, whose Node entrypoints remain CommonJS-compatible, while `src/lib/octokit-loader.ts` still loads the clients lazily so unrelated public routes do not initialize GitHub clients unnecessarily. Both the shared GitHub helper and GitHub App helper use that loader. This keeps unrelated public routes such as `/api/config`, `/api/session`, and `/api/themes` from crashing during function initialization; it does not imply that GitHub write credentials are configured.

If a future deployment reports the same error, first inspect the deployed lockfile/package metadata and confirm that the CommonJS-compatible Octokit versions are installed. Then inspect the compiled helper for an unexpected ESM-only package version or a top-level import that bypasses `src/lib/octokit-loader.ts`, followed by a fresh Vercel deployment rather than relying on a previously cached function bundle.
