...
(function (global) {
    ...
    function detectEmbedType(name) {
        ...
        for (var t in EMBED_EXTS) {
            if (Object.prototype.hasOwnProperty.call(EMBED_EXTS, t) && EMBED_EXTS[t].test(clean))
                return t;
        }
        return 'unknown';
    }
    ...
    function splitInlineText(child, state, re, makeHtml) {
        ...
        for (i = 0; i < parts.length; i++) {
            if (i % 2 === 1) {
                ...
                if (html === null) {
                    t = new state.Token('text', '', 0);
                    t.content = parts[i];
                    result.push(t);
                }
                else {
                    t = new state.Token('html_inline', '', 0);
                    t.content = html;
                    result.push(t);
                }
            }
            ...
            for (i = 0; i < state.tokens.length; i++) {
                ...
                bt.children = processInline(bt.children, state, function (child, st) {
                    return splitInlineText(child, st, /(?<![&\w#])(#[a-zA-Z_][a-zA-Z0-9_/\-]*)/g, function (m) {
                        ...
                        if (/^#[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/.test(m))
                            return null;
    ...
    function ruleMathBlockPromote(md) {
        md.core.ruler.push('obs_math_block_promote', function (state) {
            ...
            for (var i = 0; i < tokens.length; i++) {
                ...
                var prev = tokens[i - 1];
                var next = tokens[i + 1];
                if (!prev || prev.type !== 'paragraph_open')
                    continue;
                if (!next || next.type !== 'paragraph_close')
                    continue;
    ...
    function ruleMathRawDelimiters(md) {
        function mathRawInline(state, silent) {
            ...
            if (src.charCodeAt(pos) !== 0x5C /* \ */ || src[pos + 1] !== '(')
                return false;
    ...
    function ruleMathInlineDisplay(md) {
        function mathInlineDouble(state, silent) {
            ...
            if (closeIdx === -1 || closeIdx + 2 > max)
                return false;
            if (closeIdx === pos + 2)
                return false; /* "$$$$" — empty, not real math */
            if (!silent) {
    ...
    function ruleMathInline(md) {
        function mathInlineSingle(state, silent) {
            ...
            if (src.charCodeAt(pos + 1) === 0x24)
                return false; /* leave "$$" to the display rule */
            ...
            if (closeIdx === -1 || closeIdx === pos + 1)
                return false; /* no close, or empty "$$" */
            if (!silent) {
    ...
    function ruleMathBlock(md) {
        md.block.ruler.before('fence', 'obs_math_block', function (state, startLine, endLine, silent) {
            ...
            while (nextLine < endLine) {
                ...
                if (state.src.slice(lpos, lmax).trim() === '$$')
                    break;
                nextLine++;
            }
            if (nextLine >= endLine)
            ...
            tok2.content = content;
            tok2.map = [startLine, nextLine + 1];
        ...
        md.core.ruler.push('obs_callout_annotate', function (state) {
            ...
            for (i = 0; i < tokens.length; i++) {
                ...
                if (inlineTok.children && inlineTok.children[0]) {
                    ...
                    if (!inlineTok.children[0].content)
                        inlineTok.children.shift();
                }
        ...
        md.renderer.rules.blockquote_close = function (tokens, idx, options, env, self) {
            ...
            for (i = idx; i >= 0; i--) {
                ...
                if (tokens[i].type === 'blockquote_open') {
                    depth--;
                    if (depth === 0) {
                        if (tokens[i].meta && tokens[i].meta.callout)
                            return '</div></div>';
                        break;
                    }
                }
    ...
    function parseFrontmatter(raw) {
        var m = raw.match(/^---[ \t]*\r?\n([\s\S]*?)\r?\n---[ \t]*(\r?\n|$)/);
        if (!m)
            return { content: raw, frontmatter: {}, rawFrontmatter: '' };
        var rawFM = m[1];
    ...
    function _sizeMermaidSVG(svg) {
        ...
        if (w > 1 && h > 1 && wAttr && wAttr.indexOf('%') === -1)
            return;
        var vb = svg.getAttribute('viewBox');
        if (!vb)
            return;
    ...
    function _wrapMermaidDiagram(el) {
        ...
        if (!svg || el.querySelector('.mermaid-viewer-wrap'))
            return;
        _sizeMermaidSVG(svg);
        ...
        if (!svgH) {
            ...
            if (_vb) {
                var _p = _vb.trim().split(/[\s,]+/);
                svgH = parseFloat(_p[3]) || 0;
            }
        }
        if (!svgH)
        ...
        while (el.firstChild)
            canvas.appendChild(el.firstChild);
    ...
    function initMermaid(root) {
        if (typeof mermaid === 'undefined')
            return;
        var els = Array.from((root || document).querySelectorAll('.obsidian-mermaid:not([data-mermaid-processed])'));
        if (!els.length)
            return;
                ...
                try {
                    ...
                    state.expressions.list.forEach(function (expr) {
                        ...
                        if (lock === 'none') {
                            ...
                            if (!isSlider)
                                expr.secret = true; // hide only non-slider lines
                        }
                        else if (lock === 'sliders') {
                            if (!isSlider)
                                expr.secret = true; // hide formulas
                            if (isSlider)
                                expr.readonly = true; // freeze sliders but keep visible
                        }
                        else if (lock === 'all') {
    ...
    function _wrapCodeBlock(pre) {
        ...
        if (pre.parentNode && pre.parentNode.classList &&
            pre.parentNode.classList.contains('code-block-shell'))
            return;
        ...
        if (!lang && code.dataset.highlighted) {
            code.classList.forEach(function (c) {
                if (c !== 'hljs' && c !== 'language-plaintext')
                    lang = lang || c;
            });
        }
    ...
    function obsidianPlugin(md, options) {
        ...
        if (opts.enableMath)
            ruleMathBlockPromote(md); /* must run before any other custom core rule */
        if (opts.enableComments)
            ruleComments(md);
        if (opts.enableMath) {
            ruleMathRawDelimiters(md);
            ruleMathBlock(md);
        ...
        if (opts.enableStrikethrough)
            ruleStrikethrough(md);
        if (opts.enableBlockIds)
            ruleBlockIds(md);
        if (opts.enableTags)
            ruleTags(md, opts);
    ...
    global.obsidianInitTikz = function (root) {
        ...
        function _tikzSizeSVG(svg) {
            ...
            if (!svgW)
                svgW = parseFloat(svg.getAttribute('width')) || 300;
            if (!svgH)
                svgH = parseFloat(svg.getAttribute('height')) || 300;
            ...
            var maxW, baseW;
            if (ratio < 0.8)
                maxW = 375; /* tall/portrait  — less wide */
            else if (ratio > 1.25)
                maxW = 600; /* wide/landscape — more wide */
            else
                maxW = 450; /* squarish — balanced */
        ...
        function _tikzPanZoom(wrap, canvas, svg) {
            ...
            function elasticClamp(v, lo, hi) {
                if (v < lo)
                    return lo + (v - lo) * ELASTIC;
                if (v > hi)
                    return hi + (v - hi) * ELASTIC;
                return v;
            }
            function zoomAt(factor, cx, cy) {
                var rect = wrap.getBoundingClientRect();
            ...
            canvas.addEventListener('touchmove', function (e) {
                if (e.touches.length === 1 && dragging) {
                    ...
                    var d = Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY);
                    zoomAt(d / lastTouchDist, (e.touches[0].clientX + e.touches[1].clientX) / 2, (e.touches[0].clientY + e.touches[1].clientY) / 2);
                    lastTouchDist = d;
                }
            }, { passive: true });
            canvas.addEventListener('touchend', function () {
            ...
            function fadeHint() { if (hint)
                hint.classList.add('tikz-hint-gone'); }
        ...
        function _wrapTikZ(wrapper, svg) {
            if (wrapper.querySelector('.tikz-panzoom-wrap'))
                return;
        ...
        function extractPreamble(raw) {
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                if (/^\\usepackage\b/.test(trimmed) || /^\\usetikzlibrary\b/.test(trimmed)) {
        ...
        el.querySelectorAll('.tikz-source:not([data-tikz-rendered])').forEach(function (div) {
                ...
                for (var m = 0; m < mutations.length; m++) {
                    ...
                    for (var n = 0; n < added.length; n++) {
                        ...
                        if (node.nodeName === 'svg' || (node.querySelector && node.querySelector('svg'))) {
                            ...
                            if (ld2)
                                ld2.parentNode.removeChild(ld2);
                        }
                    }
                }
            });
            ...
            wrapper.addEventListener('tikzjax-load-finished', function cleanup() {
                ...
                if (svg) {
                    ...
                    if (ld3)
                        ld3.parentNode.removeChild(ld3);
