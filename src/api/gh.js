...
async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'getFile') {
            ...
            try {
                const data = await (0, _shared_1.readRepoFile)(filePath, process.env.GITHUB_BRANCH || 'main');
        ...
        if (action === 'getFileContent') {
            ...
            try {
                ...
                if (error?.status === 404)
                    return res.status(200).json({ sha: null, content: null });
                throw error;
            }
        }
        if (action === 'putFile') {
            ...
            if (!token)
                return res.status(403).json({ error: 'Write access requires GITHUB_TOKEN or GITHUB_PAT.' });
