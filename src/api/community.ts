...
export async function createPost(req: Request, res: Response) {
  ...
  try {
    ...
    try {
      if (process.env.GITHUB_REPO && (process.env.GITHUB_TOKEN || process.env.GITHUB_PAT)) {
        const repoCfg = (process.env.GITHUB_REPO || '').split('/');
        if (repoCfg.length === 2) {
          const octokit = await getOctokit({ allowUnauthenticated: false });
          // Create a discussion in the configured repo under category 'Community' if available
          const [owner, repo] = repoCfg;
          const discussion = await octokit.rest.discussions.create({ owner, repo, title, body: finalBody, category_name: 'Community' as any }).catch(() => null);
    ...
    if (isDbConfigured()) {
      const r = await dbQuery('INSERT INTO community_posts(author_email, title, body, github_discussion_id) VALUES($1,$2,$3,$4) RETURNING id, author_email, title, body, status, github_discussion_id, created_at', [email, title, finalBody, discussionId]);
      return res.status(201).json({ post: r.rows[0] });
    }

    const post = { id: inMemoryPosts.length + 1, author_email: email, title, body: finalBody, status: 'pending', github_discussion_id: discussionId, created_at: new Date().toISOString() };
...
export async function approvePost(req: Request, res: Response) {
  ...
  try {
    ...
    let merged = false;
    try {
      if (!discussionId && process.env.GITHUB_REPO) {
        const repoCfg = (process.env.GITHUB_REPO || '').split('/');
        if (repoCfg.length === 2) {
          const [owner, repo] = repoCfg;
          try {
            const gh = await import('../lib/github-app');
            const discussion = await gh.createDiscussionForRepo(owner, repo, post.title, post.body, 'Community');
      ...
      // Enable by setting GITHUB_APP_AUTO_PR=true and GITHUB_REPO=owner/repo
      if (process.env.GITHUB_REPO && process.env.GITHUB_APP_AUTO_PR === 'true') {
        const repoCfg = (process.env.GITHUB_REPO || '').split('/');
        if (repoCfg.length === 2) {
          const [owner, repo] = repoCfg;
          try {
