...
self.addEventListener('install', event => {
    event.waitUntil(caches.open(CACHE_VERSION).then(cache => Promise.allSettled(APP_SHELL.map(url => cache.add(url).catch(() => { })))));
    self.skipWaiting();
});
...
self.addEventListener('fetch', event => {
    ...
    if (request.mode === 'navigate') {
        event.respondWith(fetch(request)
            ...
            if (cached)
                return withExtraHeaders(cached, COOP_COEP_HEADERS);
